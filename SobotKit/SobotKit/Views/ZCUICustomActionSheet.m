...
#pragma mark -- 计算首次 显示的内容大小 计算提交按钮的位置
    ...
    [_commitBtn addTarget:self action:@selector(submitSuggest) forControlEvents:UIControlEventTouchUpInside];
...
-(void)sendComment{
    ...
    if ([[ZCStoreConfiguration getZCParamter:KEY_ZCISOFFLINE] intValue] == 1 || [[ZCPlatformTools sharedInstance] getPlatformInfo].config.isArtificial) {
        // 评价过客服了，下次不能再评价人工了
...
//    [dict setObject:[NSString stringWithFormat:@"%d",isresolve] forKey:@"isresolve"];
    ...
    [[[ZCUIConfigManager getInstance] getZCAPIServer] doComment:dict];
    if(isKeyBoardShow){
        isKeyBoardShow=NO;
        [_textView resignFirstResponder];
    }
