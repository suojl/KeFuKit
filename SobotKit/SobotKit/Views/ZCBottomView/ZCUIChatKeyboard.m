...
- (void)createMoreView:(BOOL)isRobot {
    ...
    if (isRobot) {
        ...
        // 添加查询订单按钮
        if ([ZCUIConfigManager getInstance].kitInfo.isShowOrderButton) {
            [titles addObject:ZCSTLocalString(@"订单")];
            [tags addObject:[NSString stringWithFormat:@"%zd", ZCKeyboardOnClickQueryOrderForGoods]];
        }
        [titles addObject:ZCSTLocalString(@"图片")];
        [titles addObject:ZCSTLocalString(@"评价")];
        [tags addObject:[NSString stringWithFormat:@"%zd", ZCKeyboardOnClickAddPhotoPicture]];
        [tags addObject:[NSString stringWithFormat:@"%zd", ZCKeyboardOnClickSatisfaction]];
        // 添加查询售后订单按钮
        if ([ZCUIConfigManager getInstance].kitInfo.isShowAfterSaleButton) {
            [titles addObject:@"售后订单"];
            [tags addObject:
